...
const EMAIL_USER = process.env.EMAIL_USER; // Email ของบริษัท
const EMAIL_PASS = process.env.EMAIL_PASS; // App Password ของบริษัท
const USE_GMAIL_SERVICE = process.env.USE_GMAIL_SERVICE;
const SMTP_HOST = process.env.SMTP_HOST;
const SMTP_PORT = process.env.SMTP_PORT;
const SMTP_SECURE = process.env.SMTP_SECURE;

console.log('🏢 ทดสอบการตั้งค่า Email ของบริษัท');
console.log('Email ของบริษัท:', EMAIL_USER);
...
if (!EMAIL_USER || !EMAIL_PASS || 
  ...
  console.log('   2. เปลี่ยน EMAIL_USER เป็น email ของบริษัท');
  console.log('   3. เปลี่ยน EMAIL_PASS เป็น App Password ของบริษัท');
  console.log('   4. ดูรายละเอียดใน BUSINESS_EMAIL_SETUP.md');
  process.exit(1);
}
...
async function testBusinessEmail() {
  ...
  try {
    ...
    console.log('✅ การเชื่อมต่อ Email ของบริษัทสำเร็จ!');
    
    console.log('🔄 กำลังส่ง email ไปยังลูกค้า...');
    const mailOptions = {
      from: `"Furniture KaoKai" <${EMAIL_USER}>`, // ส่งจาก email ของบริษัท
